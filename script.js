document.getElementById('searchBtn').addEventListener('click', function () {
    ...
    fetch(apiUrl)
        ...
        .then((data) => {
            ...
            cardsContainer.classList.add('cards-container', 'd-flex', 'flex-wrap', 'justify-content-start');
            ...
            for (let i = 0; i < maxHours; i++) {
                ...
                cardTitle.textContent = `${timestamp.getDate()}-${(timestamp.getMonth() + 1).toString().padStart(2, '0')}-${timestamp.getFullYear()}`;
                ...
                cardText.innerHTML = `
                    <p>Time: ${timestamp.getHours().toString().padStart(2, '0')}:${timestamp.getMinutes().toString().padStart(2, '0')}</p>
                    <p>Temperature: ${hourlyForecast.temperature_2m[i]}°C</p>
                    <p>Wind Speed: ${(hourlyForecast.windspeed_10m[i] * 3.6).toFixed(0)} km/h</p>
                `;
            ...
            prevButton.classList.add('btn', 'btn-primary', 'prev-button', 'd-flex', 'align-items-center');
            prevButton.innerHTML = '&lt;';
            prevButton.disabled = true;
            const nextButton = document.createElement('button');
            nextButton.classList.add('btn', 'btn-primary', 'next-button', 'd-flex', 'align-items-center');
            ...
            function updateCards() {
                ...
                nextButton.disabled = currentPage === Math.ceil(maxHours / cardsPerPage);
            }
